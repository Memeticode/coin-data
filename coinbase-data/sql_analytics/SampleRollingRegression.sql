...
With RegMetrics1 as (
	...
	-- 21 period regression
		, reg21_n			= sum(1.0)														over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_y_sum		= sum(a.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_y_bar		= avg(a.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_ysq_sum		= sum(square(a.pct_change_est_avg_price))						over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_y_stdev		= stdev(a.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_y_var		= var(a.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_x_sum		= sum(b.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_x_bar		= avg(b.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_xsq_sum		= sum(square(b.pct_change_est_avg_price))						over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_x_stdev		= stdev(b.pct_change_est_avg_price)								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_x_var		= var( b.pct_change_est_avg_price )								over (order by a.[timestamp] rows between 7 preceding and current row)
		, reg21_xy_sum		= sum(a.pct_change_est_avg_price * b.pct_change_est_avg_price)	over (order by a.[timestamp] rows between 7 preceding and current row)
